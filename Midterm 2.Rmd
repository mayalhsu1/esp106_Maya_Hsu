...
```{r}
e <- ext(73.5577, 134.7739, 18.15931, 53.56086) #used ext() to find the extent of the SpatVector and created an object with this extent
land <- crop(land, e) #cropped the SpatRaster using the extent province SpatVector

empty <- rast(, extent = e) #creating an empty raster
provincer <- rasterize(provinces, empty) #creating a raster of the province data with the empty raster
provincere <- resample(provincer, land) #resampling so that both rasters have the same extent and resolution
land1 <- mask(land, provincere) #cells in provincere that are outside China will be labeled as NA; the mask() function assigns cells in land to NA that are NA in the provincere raster
...
plot(land1[[1]]) #plotting the firs layer of the land raster
plot(provinces, add = TRUE) #plotting the original province data on top
title("Urban") #labelling the plot
...
par(mar=c(4,4,4,6)+.1) #setting margins
