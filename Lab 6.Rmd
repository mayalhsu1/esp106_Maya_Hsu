...
```{r}
observ <- is.related(bluew, mpa, "intersects")
...
which(observ == TRUE)
sub <- bluew[observ]
length((unique(sub$ind_ident)))/length((unique(bluew$ind_ident)))
```
...
```{r}
d <- distance(r, wrld, unit = "km")

s <- distance(bluew, wrld, unit = "km")
plot(d)
```
