...
```{r}
library(terra)
names <- list.files("week 7/Data/Climate Model Data_Historic", patterns = "nc$", full.name = TRUE)
...
cities <- vect(ne_download(scale = 110, "populated_places", returnclass = "sf"))
as.numeric(cities$POP2020)
pop20 <- cities[order(cities$POP2020, decreasing = TRUE)]
pop20 <- pop20[1:10]
topnames <- pop20$NAME[1:10]
pop20$NAME[1:10]
```
...
```{r}
historic_1920 <- historic$TREFHT_1
plot(historic_1920)
plot(pop20, add = TRUE)
```
...
time_index <- time(historic, format = "years")
historic_subset <- subset(historic, time_index >= 1991 & time_index <= 2000)
historic_subset <- historic_subset - 273.15
```
...
```{r}
city_obs <- extract(observed, pop20, ID = FALSE)
...
colnames(city_obs) <- topnames
colnames(city_sim) <- topnames
city_obs$time <- time(observed, format = "years")
city_sim$time <- time(historic_subset, format = "years")
```
